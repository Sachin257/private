...
<script language="javascript" type="text/javascript">
   ...
   var tinymce = {
	...
	function saveDraftCKE(preview) {
        ...
        if(draft_true != "false")
            {
        if(check_postid[1] == <?php if($_SESSION['user']->blogbody != "undefined" && isset($_SESSION['user']->blogbody) && $match == true) { $xp = explode("~^|!#", $_SESSION['user']->blogbody); echo $xp[1]; } else { echo "0"; } ?>)
        {
            $(document).ready(function(){
            $("input[name='blogtitle']").val("<?php  echo $_SESSION['user']->blogtitle; ?>");
            $("input[name='blogtags']").val("<?php  echo$_SESSION['user']->blogtags; ?>");
            });
        }
        }
